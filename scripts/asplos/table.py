...
def generate_table_amd(summary, out_path, mode, device_list, headings):
    # Prepare headers
    summary = summary.query(f"mode == '{mode}'")

    header_models = []
    header_batches = []
    for model, batches in headings:
        header_models.extend([MODEL_CONFIGS[model]["name"], ""])
        header_batches.extend(batches)

    with open(out_path, "w") as file:
        # Write headers
        write_csv_row(file, ["",] + header_models, include_comma=False)
        write_csv_row(file, ["Batch Size",] + header_batches, include_comma=False)

        for device in device_list:
            row = [f"{DEVICE_MAP[device]}"]
            for model, batches in headings:
                for batch in batches:
                    # NeuSight Prediction
                    condition = (
                    ...
                    entry = summary.loc[condition, 'neusight_all_e2e_err']
                    item = format_error(entry.values[0] if not entry.empty else None)
                    if item != "":
                        item += "%"
                    row.append(item)
...
def table_amd_inf(summary, out_path):
    ...
    generate_table_amd(summary, out_path, "inf", device_list, headings)
...
def table_amd_train(summary, out_path):
    ...
    generate_table_amd(summary, out_path, "train", device_list, headings)


def table_perop(summary, out_path):
    # only nvidia gpus
    file = open(out_path, "w")
    summary = summary[summary['option'].isnull()]
    # device name not include amd
    summary = summary[~summary['device'].str.contains("AMD")]
    # neusight_all_e2e_latency is not null
    summary = summary[summary['neusight_all_e2e_latency'].notnull()]
...
def table_distributed(summary, out_path):
    ...
    parallel_options = ["dp4", "tp4", "pp4_1"]
